    ...
    private func presentTransition(transitionContext: UIViewControllerContextTransitioning) {
        ...
        case .right:
            toView.frame = CGRect(x: contentView.bounds.size.width, y: 0, width: contentView.bounds.size.width / 2, height:
                contentView.bounds.size.height)
        case .center:
            toView.frame = CGRect(x: 0, y: contentView.bounds.size.width / 12, width: contentView.bounds.size.width / 2, height:
                contentView.bounds.size.height)
            toView.alpha = 0
        }
        ...
        UIView.animate(withDuration: duration, animations: { [weak self] in
            ...
            case .right:
                fromView.frame = CGRect(x: contentView.bounds.size.width, y: 0, width: contentView.bounds.size.width, height:
                    contentView.bounds.size.height)
            case .center:
                fromView.frame = CGRect(x: 0, y: 0, width: contentView.bounds.size.width, height:
                    contentView.bounds.size.height)
